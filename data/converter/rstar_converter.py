...
class Converter_Rstar:
    ...
    async def handle_title(
        cc: CustomCrypto,
        write_date_rdr2: bool = False,
        clear_rdr2_pc_chks: bool = False,
    ) -> None:
        unix = int(time.time())
        date = time.strftime("%d/%m/%y %H:%M:%S", time.gmtime(unix))
        if write_date_rdr2 or clear_rdr2_pc_chks:
            TITLE = f"@!HTOS CONVERTER!@ - {date}".encode(
                "utf-16le"
            )  # buffersize 0x100
        else:
            TITLE = f"~b~HTOS CONVERTER ~p~∑ ~g~‹ - {date}".encode(
                "utf-16le"
            )  # buffersize 0x100
        ...
        try:
            ...
            if header == crypt.GTAV_HEADER and platform == "ps4":
                async with Converter(filepath, False) as cv:
                    await cv.push_bytes(
                        crypt.GTAV_PS_HEADER_OFFSET, crypt.GTAV_PC_HEADER_OFFSET
                    )
                ...
                async with Converter(filepath, False) as cv:
                    await cv.push_bytes(
                        crypt.GTAV_PC_HEADER_OFFSET, crypt.GTAV_PS_HEADER_OFFSET
                    )
                    await Converter_Rstar.handle_title(cv)
    ...
    async def convert_file_RDR2(filepath: str) -> None:
        try:
            ...
            if header == crypt.RDR2_HEADER and platform == "ps4":
                async with Converter(filepath, False) as cv:
                    await cv.push_bytes(
                        crypt.RDR2_PS_HEADER_OFFSET, crypt.RDR2_PC_HEADER_OFFSET
                    )
                ...
                async with Converter(filepath, False) as cv:
                    await cv.push_bytes(
                        crypt.RDR2_PC_HEADER_OFFSET, crypt.RDR2_PS_HEADER_OFFSET
                    )
