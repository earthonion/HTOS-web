...
class Crypt_Mhwi:
    ...
    class Mhwi(CC):
        ...
        async def decrypt_region(self, offset: int, length: int) -> None:
            ...
            for i in range(32):
                ...
                while save_offset < key_length[i] + offset:
                    ...
                    salt_val_0 = struct.unpack(
                        "<I", salt[salt_idx_0 * 4 : salt_idx_0 * 4 + 4]
                    )[0]
                    salt_val_1 = struct.unpack(
                        "<I", salt[salt_idx_1 * 4 : salt_idx_1 * 4 + 4]
                    )[0]
                    salt_val_2 = struct.unpack(
                        "<I", salt[salt_idx_2 * 4 : salt_idx_2 * 4 + 4]
                    )[0]
                    salt_val_3 = struct.unpack(
                        "<I", salt[salt_idx_3 * 4 : salt_idx_3 * 4 + 4]
                    )[0]
        ...
        async def encrypt_region(self, offset: int, length: int, saveslot: int) -> None:
            ...
            key_salt = (
                self.zlib.crc32(checksum, 0xC0490023 ^ 0xFF_FF_FF_FF) ^ 0xFF_FF_FF_FF
            )
        ...
        async def generate_checksum(
            self, offset: int, length: int, saveslot: int
        ) -> bytes:
            param_1_offset = 0xC0490023

            # Initialize auStack_68 array
            auStack_68 = [0x67308D52, 0x26AE9DAB, 0x701D5D68, param_1_offset]
            ...
            uStack_88 = (
                int(
                    (
                        Crypt_Mhwi.FLOATS[
                            (
                                Crypt_Mhwi.BLOWFISH_SBOX[auStack_58[0] & 0xFFF]
                                + auStack_58[0]
                            )
                            & 0xFFF
                        ]
            ...
            crc32 = self.create_ctx_crc32(
                uint32(auStack_58[0] ^ 0xFF_FF_FF_FF, "little", const=True)
            )
            ...
            for i in range(1, 8):
                ...
                crc32 = self.create_ctx_crc32(
                    uint32(init_crc ^ 0xFF_FF_FF_FF, "little", const=True)
                )
            ...
            for i in range(4):
                auStack_c8[i] = (
                    auStack_a8[i] ^ auStack_58[i] ^ auStack_68[3] ^ auStack_a8[7]
                )
            ...
            for val in auStack_58:
                for byte_idx in range(4):
                    ...
                    uVar8 = (uVar8 >> 8) ^ Crypt_Mhwi.CRC_TABLE[
                        byte_val ^ (uVar8 & 0xFF)
                    ]
        ...
        def generate_salt(key_salt: int) -> bytes:
            ...
            salt[0:4] = struct.pack(
                "<I", Crypt_Mhwi.BLOWFISH_SBOX[first_offset] ^ 0x4BF0CF23 ^ key_salt
            )
            ...
            for i in range(4, 0x200, 4):
                ...
                salt[i : i + 4] = struct.pack(
                    "<I", Crypt_Mhwi.BLOWFISH_SBOX[table_offset] ^ 0x4BF0CF23 ^ key_salt
                )
        ...
        def generate_keys(key_salt: int, salt: bytes) -> list[bytes]:
            ...
            for i in range(0x20):
                ...
                key[0:4] = struct.pack(
                    "<I", Crypt_Mhwi.BLOWFISH_SBOX[offset0] ^ uVar9 ^ 0x701D5D68
                )
                key[4:8] = struct.pack(
                    "<I", Crypt_Mhwi.BLOWFISH_SBOX[offset1] ^ uVar9 ^ 0x4BF0CF23
                )
                key[8:12] = struct.pack(
                    "<I", Crypt_Mhwi.BLOWFISH_SBOX[offset2] ^ uVar9 ^ 0x7C8B3AF0
                )
                key[12:16] = struct.pack(
                    "<I", Crypt_Mhwi.BLOWFISH_SBOX[offset3] ^ uVar9 ^ 0x67308D52
                )
        ...
        def generate_key_length(key_salt: int, length: int) -> list[int]:
            ...
            for i in range(31):
                ...
                calc_len = (
                    int(variation * average_length_float) + accumulator
                ) & 0xFF_FF_FF_F0
