...
class CustomCrypto:
    ...
    async def read(self, stop_off: int = -1, backwards: bool = False) -> int:
        if not backwards:
            ...
            self.chunk = bytearray(
                await self.r_stream.read(self.chunk_end - self.chunk_start)
            )
    ...
    async def find(
        self, d: bytes | bytearray, start_off: int = 0, end_off: int = -1
    ) -> int:
        if end_off < 0:
            end_off = self.size
    ...
    async def rfind(
        self, d: bytes | bytearray, start_off: int = -1, end_off: int = 0
    ) -> int:
        if start_off < 0:
            start_off = self.size
    ...
    async def fraction_byte(
        self, byte: uint8 = uint8(0, const=True), div: int = 2
    ) -> bool:
        assert div != 0
    ...
    def create_ctx_mmh3_u32(
        self, seed: uint32 = uint32(0, "little", const=True)
    ) -> int:
        seed = uint32(seed.value, seed.ENDIANNESS)
    ...
    def create_ctx_hmac(
        self, key: bytes | bytearray, digestmod: hmac._DigestMod
    ) -> int:
    ...
    async def obtain_files(
        ...
        for entry in filelist:
            ...
            if await aiofiles.os.path.isfile(entry_path) and entry not in exclude:
                ...
                entry_path
            ) and entry_path != os.path.join(path, SCE_SYS_NAME):
