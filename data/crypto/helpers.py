        ...
        if title_id in GTAV_TITLEID:
            ...
            if choice:
                await Crypto.Ndog.check_dec_ps(
                    destination_directory, Crypto.Ndog.START_OFFSET
                )

        elif title_id in NDOG_COL_TITLEID:
            if choice:
                await Crypto.Ndog.check_dec_ps(
                    destination_directory, Crypto.Ndog.START_OFFSET_COL
                )
            ...
            title_id in DL1_TITLEID
            or title_id in DL2_TITLEID
            or title_id in DI1_TITLEID
        ):
            if choice:
...
async def extra_import(
    ...
    try:
        if title_id in GTAV_TITLEID:
            await Crypto.Rstar.check_enc_ps(
                filepath, Crypto.Rstar.GTAV_PS_HEADER_OFFSET
            )

        elif title_id in RDR2_TITLEID:
            await Crypto.Rstar.check_enc_ps(
                filepath, Crypto.Rstar.RDR2_PS_HEADER_OFFSET
            )
