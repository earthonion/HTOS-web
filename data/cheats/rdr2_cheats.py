...
class Cheats_RDR2:
    ...
    async def init_savefile(filepath: str) -> str:
        ...
        if encrypted:
            start_offset = (
                crypt.RDR2_PS_HEADER_OFFSET
                if platform == "ps4"
                else crypt.RDR2_PC_HEADER_OFFSET
            )
            try:
    ...
    async def change_money(
        filepath: str, money: int, platform: Literal["ps4", "pc"]
    ) -> None:
    ...
    def loaded_embed(stats: dict[str, str]) -> discord.Embed:
        ...
        emb.description = emb.description.format(
            platform=stats["Platform"], money=stats["Money"]
        )
        return emb
