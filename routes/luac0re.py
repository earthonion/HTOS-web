...
PREBUILT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "prebuilt", "luac0re"
)
    ...
    if request.method == "POST":
        ...
        if not profile_id:
            ...
            return await render_template(
                "luac0re.html", profiles=profiles, versions=VERSIONS
            )
        ...
        save_name = info["save"]
        shutil.copy2(
            os.path.join(src_dir, save_name), os.path.join(upload_dir, save_name)
        )
        shutil.copy2(
            os.path.join(src_dir, f"{save_name}.bin"),
            os.path.join(upload_dir, f"{save_name}.bin"),
        )
