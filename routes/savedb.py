...
from services.files import DangerousFileError, check_dangerous_files, check_zip_safety
...
async def contribute():
    if request.method == "POST":
        ...
        try:
            await db.execute(
                "UPDATE savedb_entries SET save_path = ? WHERE id = ?", (save_dir, entry_id)
            )
            await db.commit()
        finally:
            await db.close()
...
async def detail(entry_id):
    ...
    try:
        ...
        # Get user's vote
        cursor = await db.execute(
            "SELECT vote FROM savedb_votes WHERE entry_id = ? AND user_id = ?", (entry_id, user_id)
        )
...
async def download(entry_id):
    ...
    try:
        cursor = await db.execute(
            "SELECT save_path, title, title_id FROM savedb_entries WHERE id = ?", (entry_id,)
        )
        entry = await cursor.fetchone()
    finally:
        await db.close()
...
async def delete(entry_id):
    ...
    try:
        cursor = await db.execute(
            "SELECT save_path FROM savedb_entries WHERE id = ? AND user_id = ?", (entry_id, user_id)
        )
        entry = await cursor.fetchone()
        if not entry:
