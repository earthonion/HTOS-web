...
from utils.extras import generate_random_string, zipfiles
...
class TimeoutHelper:
    ...
    async def handle_timeout(
        self, ctx: discord.ApplicationContext | discord.Message
    ) -> None:
    ...
    async def callback(
        ...
        if current_instance is not None:
            await interaction.response.send_message(
                "You can not create a new thread when you already have an active instance!",
                ephemeral=True,
            )
            return
        ...
        try:
            thread = await interaction.channel.create_thread(
                name=interaction.user.name, auto_archive_duration=10080
            )
...
def upl_check(message: discord.Message, ctx: discord.ApplicationContext) -> bool:
    if message.author == ctx.author and message.channel == ctx.channel:
        return (
            ...
            or (message.content and gdapi.is_google_drive_link(message.content))
            or (message.content and message.content == "EXIT")
        )
...
def accid_input_check(
    message: discord.Message, ctx: discord.ApplicationContext
) -> bool:
    if message.author == ctx.author and message.channel == ctx.channel:
        return (message.content and checkid(message.content)) or (
            message.content and message.content == "EXIT"
        )
...
async def wait_for_msg(
    ctx: discord.ApplicationContext,
    check: Callable[[discord.Message, discord.ApplicationContext], bool],
    embed: discord.Embed | None,
    delete_response: bool = False,
    timeout: int | None = OTHER_TIMEOUT,
) -> discord.Message:
    try:
        response = await bot.wait_for(
            "message", check=lambda message: check(message, ctx), timeout=timeout
        )
        if delete_response:
...
async def download_attachment(
    attachment: discord.Attachment, folderpath: str, filename: str | None = None
) -> str:
    if filename is None:
    ...
    try:
        ...
        if e.errno == errno.EINVAL:
            raise FileError(
                f"The filename {os.path.basename(filepath)} is unsupported!"
            )
        raise
...
async def task_handler(
    ...
    ordered_tasks: list[Callable[[], Awaitable[Any]]],
    ordered_embeds: list[discord.Embed],
) -> list[Any]:
    ...
    for i in range(tasks_len):
        ...
        if cancel_task is None:
            cancel_task = asyncio.create_task(
                wait_for_msg(d_ctx.ctx, exit_check, None, timeout=GENERAL_TIMEOUT)
            )
...
async def upload2(
    ...
    message = await wait_for_msg(
        d_ctx.ctx, upl_check, embUtimeout, timeout=UPLOAD_TIMEOUT
    )
    ...
    if len(message.attachments) > max_files:
        ...
        valid_attachments = await check_saves(
            d_ctx.msg,
            attachments,
            ps_save_pair_upload,
            sys_files,
            ignore_filename_check,
            savesize,
        )
        ...
        for attachment in valid_attachments:
            ...
            emb.description = emb.description.format(
                filename=attachment.filename, i=i, filecount=filecount
            )
        ...
        try:
            ...
            if opt.gd_choice == UploadGoogleDriveChoice.STANDARD:
                task = [
                    lambda: gdapi.downloadsaves_recursive(
                        d_ctx.msg,
                        folder_id,
                        save_location,
                        max_files,
                        SCE_SYS_CONTENTS if sys_files else None,
                        ps_save_pair_upload,
                        ignore_filename_check,
                        opt.gd_allow_duplicates,
                    )
                ]
            else:
                task = [
                    lambda: gdapi.downloadfiles_recursive(
                        d_ctx.msg, save_location, folder_id, max_files, savesize
                    )
                ]
            await d_ctx.msg.edit(embed=cancel_notify_emb)
            await asyncio.sleep(1)
...
async def upload1(d_ctx: DiscordContext, save_location: str) -> str:
    ...
    if len(message.attachments) == 1:
        ...
        if len(attachment.filename) > MAX_FILENAME_LEN:
            await message.delete()
            raise FileError(
                f"Filename: {attachment.filename} ({len(attachment.filename)}) is exceeding {MAX_FILENAME_LEN}!"
            )

        elif attachment.size > FILE_LIMIT_DISCORD:
            await message.delete()
            raise FileError(
                f"DISCORD UPLOAD ERROR: File size of '{attachment.filename}' exceeds the limit of {bytes_to_mb(FILE_LIMIT_DISCORD)} MB."
            )
        ...
        try:
            ...
            task = [
                lambda: gdapi.downloadfiles_recursive(
                    d_ctx.msg, save_location, folder_id, 1
                )
            ]
...
async def upload2_special(
    ...
    if len(message.attachments) > max_files:
        ...
        valid_attachments = await check_saves(
            d_ctx.msg, attachments, False, False, True, savesize
        )
        filecount = len(valid_attachments)
        if filecount == 0:
        ...
        for attachment in valid_attachments:
            ...
            path_len = len(
                MOUNT_LOCATION + f"/{'X' * RANDOMSTRING_LENGTH}/" + rel_file_path + "/"
            )
            ...
            if len(file_name) > MAX_FILENAME_LEN:
                raise FileError(
                    f"File name ({file_name}) ({len(file_name)}) is exceeding {MAX_FILENAME_LEN}!"
                )

            elif path_len > MAX_PATH_LEN:
                raise FileError(
                    f"Path: {rel_file_path} ({path_len}) is exceeding {MAX_PATH_LEN}!"
                )
            ...
            emb = embuplSuccess.copy()
            emb.description = emb.description.format(
                filename=rel_file_path, i=i, filecount=filecount
            )
            await d_ctx.msg.edit(embed=emb)
...
async def psusername(ctx: discord.ApplicationContext, username: str) -> str:
    ...
    if username == "":
        ...
        if user_id is not None:
            ...
            if search[0]:
                blacklist_logger.info(
                    f"{ctx.author.name} ({ctx.author.id}) used a blacklisted account ID: {user_id}"
                )
        ...
        if (len(username) < 3 or len(username) > 16) or (
            not bool(PSN_USERNAME_RE.fullmatch(username))
        ):
...
async def replace_decrypted(
    ...
    if upload_individually:
        ...
        for file in files:
            ...
            )  # use replace to prevent collision
        ...
        if total_count > savesize:
            raise OrbisError(
                f"The files you are uploading for this save exceeds the savesize {bytes_to_mb(savesize)} MB!"
            )
                        ...
                        if file_constructed == savefile:
                            ...
                            if await aiofiles.os.path.exists(new_path):
                                new_folder = os.path.join(
                                    local_download_path,
                                    generate_random_string(RANDOMSTRING_LENGTH),
                                )
                            ...
                            if not ignore_secondlayer_checks:
                                await extra_import(
                                    Crypto, titleid, new_path, savepairname
                                )

                            task = [
                                lambda: fInstance.replacer(new_path, cwd_here, last_N)
                            ]
                            await task_handler(d_ctx, task, [])
                            completed.append(last_N)
                            await aiofiles.os.remove(
                                new_path
                            )  # remove to decrease chance of collison
        else:
            task = [
                lambda: fInstance.upload_folder(mount_location, local_download_path)
            ]
            await task_handler(d_ctx, task, [])
...
async def send_final(
    ...
    if final_size < BOT_DISCORD_UPLOAD_LIMIT and not shared_gd_folderid:
        ...
        task = [
            lambda: gdapi.uploadzip(
                d_ctx.msg, final_file, file_name, shared_gd_folderid
            )
        ]
...
async def qr_interface_main(
    d_ctx: DiscordContext, stored_saves: dict[str, dict[str, dict[str, str]]]
) -> tuple[str, dict[str, dict[str, str]]] | tuple[Literal["EXIT"], Literal["EXIT"]]:
    game_desc = ""
    game_msgs = []
    selection = {}
    entries_added = 0
    ...
    try:
        message = await bot.wait_for(
            "message",
            check=lambda message: qr_check(message, d_ctx.ctx, entries_added, "EXIT"),
...
async def run_qr_paginator(
    d_ctx: DiscordContext, stored_saves: dict[str, dict[str, dict[str, str]]]
) -> tuple[ReturnTypes, list[str]]:
    while True:
        ...
        try:
            message: discord.Message = await bot.wait_for(
                "message",
                check=lambda message: qr_check(
                    message, d_ctx.ctx, entries_added, "BACK"
                ),
                timeout=OTHER_TIMEOUT,
            )
        except asyncio.TimeoutError:
        ...
        if message.content == "BACK":
            ...
            return ReturnTypes.SUCCESS, [
                os.path.join(selected_save, x) for x in savenames
            ]
