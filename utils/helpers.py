...
from psnawp_api.core.psnawp_exceptions import PSNAWPAuthenticationError, PSNAWPNotFoundError
...
from utils.exceptions import FileError, OrbisError, PSNIDError, TaskCancelledError, WorkspaceError
from utils.extras import generate_random_string, zipfiles
from utils.instance_lock import INSTANCE_LOCK_global
from utils.orbis import check_saves, checkid, handle_accid, parse_pfs_header, parse_sealedkey
...
class ThreadButton(discord.ui.View):
    ...
    @discord.ui.button(
        label="Create thread", style=discord.ButtonStyle.primary, custom_id="CreateThread"
    )
...
async def upload2(
    ...
    if len(message.attachments) > max_files:
        ...
        valid_attachments = await check_saves(
            d_ctx.msg, attachments, ps_save_pair_upload, sys_files, ignore_filename_check, savesize
        )
...
async def upload2_special(
    ...
    if len(message.attachments) > max_files:
        ...
        for attachment in valid_attachments:
            _, ext = os.path.splitext(attachment.filename)
            basename = (
                attachment.title + ext if attachment.title is not None else attachment.filename
            )
            basename = os.path.basename(basename)
                        ...
                        if file_constructed == savefile:
                            ...
                            if await aiofiles.os.path.exists(new_path):
                                new_folder = os.path.join(
                                    local_download_path, generate_random_string(RANDOMSTRING_LENGTH)
                                )
        ...
        try:
            task = [
                lambda: d_ctx.ctx.send(
                    content=extra_msg, file=discord.File(final_file), reference=d_ctx.msg
                )
            ]
            await task_handler(d_ctx, task, [])
        except TimeoutError:
...
def qr_check(
    message: discord.Message, ctx: discord.ApplicationContext, max_index: int, exit_val: str
) -> str | bool:
    if message.author == ctx.author and message.channel == ctx.channel:
    ...
    while True:
        ...
        try:
            message: discord.Message = await bot.wait_for(
                "message",
                check=lambda message: qr_check(message, d_ctx.ctx, entries_added, "BACK"),
                timeout=OTHER_TIMEOUT,
            )
        except TimeoutError:
