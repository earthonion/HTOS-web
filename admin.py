...
async def revoke_key(key_id):
    db = await get_db()
    try:
        cursor = await db.execute("UPDATE worker_keys SET is_active = 0 WHERE id = ?", (key_id,))
        await db.commit()
        if cursor.rowcount == 0:
...
async def revoke_all_keys(username):
    ...
    try:
        ...
        if not row:
            ...
            return
        cursor = await db.execute(
            "UPDATE worker_keys SET is_active = 0 WHERE user_id = ? AND is_active = 1", (row["id"],)
        )
        await db.commit()
...
async def savedb_delete(entry_id):
    ...
    db = await get_db()
    try:
        cursor = await db.execute("SELECT save_path FROM savedb_entries WHERE id = ?", (entry_id,))
        row = await cursor.fetchone()
        if not row:
